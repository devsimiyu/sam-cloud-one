...
```bash
sam-cloud-one$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-cloud-one$ sam local start-api
sam-cloud-one$ curl http://localhost:3000/
```
...
```bash
sam-cloud-one$ sam logs -n HelloWorldFunction --stack-name sam-cloud-one --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-cloud-one
```
